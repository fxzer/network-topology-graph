...
graph.on('node:click', (e) => {
  ...
  if (model.level === 0) {
    ...
    if (currentFocus && currentFocus.id === model.id) {
      ...
      // 找到被点击节点的后代节点
      // let childNodes = []
      const childNodes = nodes.filter(node => node.level === model.level + 1)
    ...
    // 筛选点击当前VPE的子节点
    const rootAndVpeNodes = nodes.filter(node => node.level < 2)
    ...
    if (currentFocus && currentFocus.id === model.id) {
      ...
      const curChildNodes = nodes.filter((node) => {
        let isChild = false
      ...
      // 筛选点击当前VPE的子节点的边
      const curChildEdges = edges.filter((edge) => {
...
graph.on('canvas:click', () => {
  ...
  if (highlighting) {
    ...
    const loadNodes = nodes.filter(node => node.level === 1)
    const loadEdges = edges.filter(edge => edge.source === 'root')
